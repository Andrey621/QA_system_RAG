...
from models import HealthCheckResponse, CompareAnswersResponse, QuestionRequest, AnswerQuestionResponse, \
    GetTopicsResponse, GetTopicResponse, ProcessPdfResponse, FineTuneRequest, FineTuneResponse, \
    FineTuneJobResponse, FineTuneJobStatusResponse
...
FINE_TUNE_OUTPUT_DIR = os.path.join(MODELS_DIR, "fine_tuned")
os.makedirs(FINE_TUNE_OUTPUT_DIR, exist_ok=True)
FINE_TUNE_MAX_WORKERS = int(os.getenv("FINE_TUNE_MAX_WORKERS", "1"))
training_job_manager = TrainingJobManager(max_workers=FINE_TUNE_MAX_WORKERS)
...
@app.post(
    ...
    tags=["Модели"],
    summary="Дообучить локальную модель",
    description="Запускает дообучение LLM на пользовательских примерах и возвращает путь к сохранённой копии модели.",
)
async def fine_tune_model_endpoint(request_data: FineTuneRequest):
    """Дообучить локальную модель на пользовательских данных."""
    logger.info(
        "Запрос дообучения модели от пользователя %s (%s примеров).",
        request_data.user_id,
        len(request_data.examples),
    )
    base_model_for_training = request_data.base_model_name or OLLAMA_MODEL_NAME
    output_dir = request_data.output_dir or FINE_TUNE_OUTPUT_DIR
    try:
        model_path = fine_tune_local_model(
            examples=request_data.examples,
            base_model_name=base_model_for_training,
        ...
        logger.error("Ошибка дообучения модели: %s", exc, exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Ошибка дообучения модели: {str(exc)}"
        ) from exc
    return FineTuneResponse(
        status="completed",
        run_name=os.path.basename(model_path),
        model_path=model_path,
        message=f"Модель дообучена на {len(request_data.examples)} примерах.",
    )

@app.post(
    "/fine_tune_model/async",
    response_model=FineTuneJobResponse,
    tags=["Модели"],
    summary="Поставить дообучение в очередь",
    description="Ставит задачу дообучения LLM в очередь и возвращает идентификатор для отслеживания статуса.",
)
async def enqueue_fine_tune_job(request_data: FineTuneRequest):
    """Запустить асинхронное дообучение и сразу вернуть job_id."""
    if not request_data.examples:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Список примеров пуст.")
    job_id = training_job_manager.submit_job(
        request_data,
        default_model_name=OLLAMA_MODEL_NAME,
        default_output_dir=FINE_TUNE_OUTPUT_DIR,
    )
    return FineTuneJobResponse(
        job_id=job_id,
        status="queued",
        message="Задача дообучения поставлена в очередь.",
    )

@app.get(
    "/fine_tune_model/status/{job_id}",
    response_model=FineTuneJobStatusResponse,
    tags=["Модели"],
    summary="Получить статус дообучения",
    description="Возвращает текущий статус асинхронного дообучения по job_id.",
)
async def get_fine_tune_job_status(job_id: str):
    """Получить статус конкретной задачи дообучения."""
    job_status = training_job_manager.get_status(job_id)
    ...
    return FineTuneJobStatusResponse(
        ...
        model_path=job_status.get("model_path"),
    )
